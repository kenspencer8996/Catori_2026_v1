...
public sealed class AirplaneSettings
{
    ...
    private static VisualPartConfiguration Part(string name,double x,double y,double width,double height,int z) =>
        new() { Name=name, X=x, Y=y, Width=width, Height=height, PivotX=.5, PivotY=.5, ZOrder=z };
}

public partial class AirplaneUC : UserControl,IDraggable,ICanvasDragAnchor
{
    ...
    public void FaceLeft()=>IsFacingRight=false;
    public void FaceRight()=>IsFacingRight=true;
    public void FlipHorizontal()=>IsFacingRight=!IsFacingRight;
    ...
    public Task FlyToAsync(double x,double y,TimeSpan duration,CancellationToken token=default) =>
        PartAnimator.AnimateToPoseAsync(new VisualPartPose(new Dictionary<string,double>(),
    ...
    public Task SpinPropellerAsync(double revolutions,TimeSpan duration,CancellationToken token=default) =>
        SpinPropellersAsync(revolutions, duration, token);

    public Task SpinLeftPropellerAsync(double revolutions,TimeSpan duration,CancellationToken token=default) =>
        SpinOneAsync(_settings.LeftPropeller ?? _settings.Propeller,revolutions,duration,token);

    public Task SpinRightPropellerAsync(double revolutions,TimeSpan duration,CancellationToken token=default) =>
        SpinOneAsync(_settings.RightPropeller ?? _settings.Propeller,revolutions,duration,token);
    ...
    public Task SetDoorOpenAsync(bool open,TimeSpan duration,CancellationToken token=default) =>
        _settings.Door is null ? Task.CompletedTask : PartAnimator.AnimateToPoseAsync(
            new VisualPartPose(new Dictionary<string,double>
                {{_settings.Door.Name, _settings.Door.InitialAngle + (open ? _settings.DoorOpenAngle : 0)}}), duration, token);
    ...
    private Task SpinOneAsync(VisualPartConfiguration? part,double revolutions,TimeSpan duration,CancellationToken token)
        =>part is null?Task.CompletedTask:PartAnimator.AnimateToPoseAsync(
        ...
        try
        {
            while(!token.IsCancellationRequested)
            {
                ...
                await PartAnimator.AnimateToPoseAsync(
                    ...
                    TimeSpan.FromMilliseconds(650),token);
            }
        }
    ...
    private static AirplaneSettings NormalizeSettings(AirplaneSettings settings)
    {
        ...
        VisualPartConfiguration? Part(VisualPartConfiguration? part)=>part is null?null:new VisualPartConfiguration
        {
            Name=part.Name,ImagePath=part.ImagePath,X=offsetX+(part.X-minX)*scale,Y=offsetY+(part.Y-minY)*scale,
            Width=part.Width*scale,Height=part.Height*scale,PivotX=part.PivotX,PivotY=part.PivotY,
            InitialAngle=part.InitialAngle,ZOrder=part.ZOrder,ParentPartName=part.ParentPartName
        };
