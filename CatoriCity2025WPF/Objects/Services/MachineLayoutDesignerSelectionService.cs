...
namespace CatoriApp.Objects.Services
{
    ...
    {
        public MachineLayoutDesignerWindow OpenDesignerForImage(string imagePath, Rect sourceSelection)
        {
            var window = new MachineLayoutDesignerWindow(0, imagePath, sourceSelection);
            window.Show();
            return window;
        }

        public MachineLayoutDesignerWindow CaptureSelectionAndOpen(FrameworkElement source, Rect selectionBounds)
        {
            var imagePath = CaptureSelection(source, selectionBounds);
            return OpenDesignerForImage(imagePath, selectionBounds);
        }
