...
{
    public class FactoryGeometryService
    {
        public PathGeometry BuildPathGeometry(IEnumerable<FactoryLayoutPointViewModel> points)
        {
            ...
            for (int i = 1; i < ordered.Count; i++)
            {
                ...
                {
                    case FactoryLayoutSegmentKind.QuadraticBezier:
                        figure.Segments.Add(new QuadraticBezierSegment(
                            new Point(point.Control1X ?? point.X, point.Control1Y ?? point.Y),
                            endPoint,
                            true));
                        break;
                    case FactoryLayoutSegmentKind.CubicBezier:
        ...
        public List<Point> SampleStraightPath(IEnumerable<FactoryLayoutPointViewModel> points, double spacing)
        {
            var ordered = points.OrderBy(p => p.PointIndex).ToList();
