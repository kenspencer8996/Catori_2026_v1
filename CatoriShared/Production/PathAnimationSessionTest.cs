...
public sealed class PathAnimationSessionTest:IDisposable
{
    private readonly List<PathAnimationHandleTest> _handles=[];
    ...
    public PathAnimationSessionTest()
    {
        WeakReferenceMessenger.Default
            .Register<RobotPartTransferMessage>(this,
                static (recipient,message)=>((PathAnimationSessionTest)recipient).HandlePartTransfer(message));
        WeakReferenceMessenger.Default.Register<CatoriApp.Core.Objects.Arguments.AnimationCompleteMessage>(this,
            static (recipient,message)=>((PathAnimationSessionTest)recipient).HandleAnimationComplete(message));
    }

    public IReadOnlyList<PathAnimationHandleTest> Handles=>_handles;

    public void Add(PathAnimationHandleTest handle)
    {
        ...
        foreach(var handle in _handles)
        {
                ...
                ||_deferredPaths.Contains(handle.AnimationName))
            {
                System.Diagnostics.Debug.WriteLine(
                    $"Production path '{handle.AnimationName}' is deferred until handoff.");
            }
            else
            {
                System.Diagnostics.Debug.WriteLine(
                    $"Production path '{handle.AnimationName}' is a root; starting direct control " +
                    $"'{handle.AnimationControl.GetType().FullName}'.");
                handle.Start();
            }
        }
    }
    ...
    private void HandlePartTransfer(RobotPartTransferMessage message)
    {
        ...
        if(handle.AnimationControl.Dispatcher.CheckAccess())ContinueProduction();
        else handle.AnimationControl.Dispatcher.BeginInvoke((Action)ContinueProduction);
    }
    ...
    private void HandleAnimationComplete(CatoriApp.Core.Objects.Arguments.AnimationCompleteMessage message)
    {
        ...
        if(handle.AnimationControl.Dispatcher.CheckAccess())ContinuePath();
        else handle.AnimationControl.Dispatcher.BeginInvoke((Action)ContinuePath);
    }
