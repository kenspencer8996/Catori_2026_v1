...
namespace CatoriServices.Objects.database.Robots
{
    public class MachineLayoutDesignerRepository
    {
        ...
        private static MachineLayoutDesignerEntity Map(SqliteDataReader reader) => new()
        {
            MachineLayoutDesignerId = reader.GetInt64(0), LocationId = reader.GetInt64(1), SelectionX = reader.GetDouble(2),
            SelectionY = reader.GetDouble(3), SelectionWidth = reader.GetDouble(4), SelectionHeight = reader.GetDouble(5),
            CreatedAt = DateTime.Parse(reader.GetString(6)), UpdatedAt = DateTime.Parse(reader.GetString(7))
        };
    }
}
