...
VALUES(@locationId,@cents,@type);",("@locationId",locationId),("@cents",cents),("@type",rateType.ToString()));
    ...
    public MoneyOperationResult CreditFoundMoney(int personId,decimal amount,long? locationId,string description)
        =>ChangeWallet(personId,ToPositiveCents(amount),"Found",locationId,"FoundMoney",null,description);
...
VALUES(@personId,@bankId,@cents,@cents,@rate,'Active',1);",("@personId",personId),("@bankId",bankId),("@cents",cents),("@rate",interestRate));
    ...
    private static long ReadWalletCents(SqliteConnection c,SqliteTransaction t,int id)=>ScalarLong(c,t,"SELECT CAST(ROUND(COALESCE(Funds,0)*100) AS INTEGER) FROM Person WHERE PersonID=@id;",("@id",id));
    private static long ReadBankTotalCents(SqliteConnection c,SqliteTransaction t,int id)=>ScalarLong(c,t,"SELECT CAST(ROUND(COALESCE(SUM(Amount),0)*100) AS INTEGER) FROM Deposit WHERE PersonId=@id;",("@id",id));
    private static long ReadLoanTotalCents(SqliteConnection c,SqliteTransaction t,int id)=>ScalarLong(c,t,"SELECT COALESCE(SUM(OutstandingPrincipalCents),0) FROM Loan WHERE PersonId=@id AND Status='Active';",("@id",id));
...
VALUES(@personId,@cents,@type,@locationId,@referenceType,@referenceId,@description);",("@personId",personId),("@cents",cents),("@type",type),("@locationId",locationId),("@referenceType",referenceType),("@referenceId",referenceId),("@description",description));return LastInsertId(c,t); }
    private static long LastInsertId(SqliteConnection c,SqliteTransaction t)=>ScalarLong(c,t,"SELECT last_insert_rowid();");
    ...
    public static long ToCents(decimal amount)=>checked((long)Math.Round(amount*100m,MidpointRounding.AwayFromZero));
