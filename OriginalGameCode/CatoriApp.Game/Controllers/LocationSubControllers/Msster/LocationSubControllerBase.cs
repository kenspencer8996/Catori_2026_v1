...
namespace CatoriApp.Game.Controllers.LocationSubControllers
{
    public class LocationSubControllerBase : CatoriApp.Game.Controllers.ControllerAnimateLayoutsBase
    {
        ...
        protected override bool OwnsAnimation(AnimationCompleteMessage message)
        {
            if (message.Part == null)
                return false;

            DependencyObject? current = message.Part;
            while (current != null)
            {
                if (ReferenceEquals(current, _view.MainCanvas))
                    return true;
