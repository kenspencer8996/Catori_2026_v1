...
{
    public class Location_UController
    {
        Location_UC _view;
        ...
        public Location_UController(Location_UC view, 
            int locationid)
        {
        ...
        {
            string? locationName = new CatoriServices.Objects.database.Locations.LocationRepository()
                .GetByIdAsync(locationId).GetAwaiter().GetResult()?.LocationName;
            string controllerName = ToControllerIdentifier(locationName) + "SubController";
            string typeName = $"CatoriApp.Game.Controllers.LocationSubControllers.{controllerName}";
            Type? controllerType = typeof(Location_UController).Assembly.GetType(typeName);
            if (controllerType != null)
                return CreateController(controllerType, typeName, locationId);

            typeName = $"CatoriApp.Game.Controllers.LocationSubControllers.Locationinterior{locationId}SubController";
            controllerType = typeof(Location_UController).Assembly.GetType(
                typeName,
                throwOnError: false,
                ignoreCase: true);
            if (controllerType == null)
            {
                CatoriShared.Diagnostics.GameSafetyLog.Warning("Location controller",
                    $"The generated location controller '{typeName}' was not found. Loading the location without production behavior.");
                return new PassiveLocationSubController(_view, locationId);
            }

            return CreateController(controllerType, typeName, locationId);
        }

        private ILocationSubController CreateController(Type controllerType, string typeName, int locationId)
        {
        ...
        private static string ToControllerIdentifier(string? value)
        {
        ...
        public void ProducePart()
        {
            try
            {
                _locationSubController.StartAnimation();
