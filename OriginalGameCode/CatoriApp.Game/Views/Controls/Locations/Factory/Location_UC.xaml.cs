...
namespace CatoriApp.Game.Views.Controls.Locations.Factory
{
    ...
    public partial class Location_UC : UserControl,IDropTarget
    {
        ...
        private async Task LoadLayoutVisualComponentsAsync()
        {
            ...
            foreach (LocationLayoutItemViewModel item in layoutItems)
            {
                ...
                try
                {
                    ...
                    FrameworkElement? existingVisual = MainCanvas.Children.OfType<FrameworkElement>().FirstOrDefault(control =>
                    ...
                    {
                        if (existingVisual is AirplaneUC existingAirplane)
                        {
                            _dragManager.RegisterDropTarget(existingAirplane);
                        }
                        continue;
                    }
                    ...
                    if(visual is AirplaneUC airplaneTarget)
                        _dragManager.RegisterDropTarget(airplaneTarget);
                }
                catch (Exception ex)
                {
        ...
        {
            return metadata.TryGetProperty(propertyName, out JsonElement value) && value.ValueKind == JsonValueKind.String
                ? value.GetString()
                : null;
        }
        ...
        private void TicketKiosk_BuyTicketRequested(object? sender, TicketPurchaseRequestedEventArgs e)
        {
            ...
            try
            {
                ...
                person.Role = PersonRole.Passenger;
                kiosk.PublishPurchasedTicket(ticket, person.Name);
