namespace CatoriApp.Core.Objects.Shared
{
    internal class DefaultSampleDataHelper
    {
        private static void CreateDefaultCityApp()
        {
            try
            {
                //BusinessViewModel busiess = new BusinessViewModel();
                //GlobalStuff.Addbusinessntity("4th 1st bank", 200, BusinessTypeEnum.Financial);
                //GlobalStuff.Addbusinessntity("A bank", 210, BusinessTypeEnum.Financial);
                //GlobalStuff.Addbusinessntity("My CU", 220, BusinessTypeEnum.Financial);

                //GlobalStuff.Addbusinessntity("Iron W", 140, BusinessTypeEnum.Location);
                //GlobalStuff.Addbusinessntity("Smelter", 170, BusinessTypeEnum.Location);
                //GlobalStuff.Addbusinessntity("Hot Cars", 120, BusinessTypeEnum.Carlot);

                //GlobalStuff.AddPersonViewModel("Catori", true, PersonImageTypeEnum.Normal);
                //GlobalStuff.AddPersonViewModel("Frank", false, PersonImageTypeEnum.Normal);
                //GlobalStuff.AddPersonViewModel("Jim", false, PersonImageTypeEnum.Normal);
                //GlobalStuff.AddPersonViewModel("Bob", false, PersonImageTypeEnum.Normal);

                //GlobalStuff.ReloadBadGuys();
            }
            catch (Exception ex)
            {

                throw;
            }
        }
    }
}



